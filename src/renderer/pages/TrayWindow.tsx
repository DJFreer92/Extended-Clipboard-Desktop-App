...
export const TrayWindow: React.FC<TrayWindowProps> = () => {
	...
	for (const clip of filteredClips.slice(0, 50)) {
			...
			<div className="tray-header">
				<div className="tray-title">
					...
					<div className="header-actions">
						<button className="action-btn open-btn" onClick={handleOpenApp} title="Open main app">
							<span className="icon icon-open-in-new"></span>
						</button>
					</div>
				</div>
				<div className="search-container">
					<button className="action-btn refresh-btn" onClick={handleRefresh} title="Refresh clips">
					...
					<div className="clips-list">
						{Array.from(groupedClips.entries()).map(([date, dateClips]) => (
							<div key={date} className="date-group">
								<div className="date-header" onClick={() => toggleDateSection(date)} role="button" tabIndex={0}>
								...
								{!collapsedDates.has(date) && (
									<>
										{dateClips.slice(0, 10).map((clip) => (
											...
											>
												...
												<div className="clip-metadata">
													{clip.FromAppName && <span className="app-badge">{clip.FromAppName}</span>}
										...
										{dateClips.length > 10 && <div className="more-clips">+{dateClips.length - 10} more clips</div>}
									</>
								)}
							</div>
						))}
