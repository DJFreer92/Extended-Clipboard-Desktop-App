...
export default function ClipList({
	...
	const getEffectiveTags = (clip: ClipModel): string[] => {
		...
		const effectiveTags = [...baseTags.filter((tag) => !modifications.removed.includes(tag)), ...modifications.added];
	...
	useEffect(() => {
		if (openClip) setModalShakeSkip(true);
	}, [openClip?.Id]);
						...
						<Fragment key={g.key}>
							<li
								className={`date-separator${collapsed[g.key] ? " is-collapsed" : ""}${
									hoveredSection === g.key ? " is-hovered" : ""
								}`}
										...
										>
											...
											>
												<span
													...
													style={{ background: isFav ? "#facc15" : "#4a4f58" }}
													aria-hidden="true"
												/>
											</button>
											<button
											...
											<div className="clip-right-meta">
												...
												{!isExpanded && clip.FromAppName && (
													<span className="badge badge-app" title={clip.FromAppName}>
														{clip.FromAppName}
													</span>
												)}
												{!isExpanded &&
													showTags.map((t) =>
														t === "…" ? (
															...
															<span
																key={t + "_tag"}
																className="badge badge-tag tag-removable"
																title={t}
					...
					<div className="modal" onClick={(e) => e.stopPropagation()}>
						<div className="modal-header modal-header-stacked" style={{ position: "relative" }}>
							<div className="modal-header-top">
								{onToggleFavorite && (
									<button
										...
										onMouseEnter={(e) => {
											...
											if (icon && !openClip.IsFavorite)
												icon.setAttribute("style", "background: #fff; width: 26px; height: 26px");
										}}
										onMouseLeave={(e) => {
											const icon = e.currentTarget.querySelector(".icon-star_filled");
											if (icon && !openClip.IsFavorite)
												icon.setAttribute("style", "background: #4a4f58; width: 26px; height: 26px");
										}}
									>
										<span
											className={"icon icon-star_filled"}
								...
								<h3 className="modal-title">
									...
									{(() => {
										...
										return (
											<span
												className="modal-date-meta"
												style={{ display: "inline-flex", alignItems: "center", gap: 8 }}
											>
												({label})
												<button
													type="button"
													className="icon-button copy-btn inline"
													aria-label="Copy clip"
													title="Copy clip"
													onClick={() => {
							...
							<div className="modal-tags-row">
								{openClip.FromAppName && (
									<span className="badge badge-app" title={openClip.FromAppName}>
										{openClip.FromAppName}
									</span>
								)}
								{openClip.Tags &&
								...
								<TagAddControl
									...
									onAdded={(name) => {
										if (name) {
											setOpenClip((c) =>
												c ? { ...c, Tags: c.Tags && !c.Tags.includes(name) ? [...c.Tags, name] : c.Tags || [name] } : c
											);
											handleTagAdded(openClip, name);
										}
									}}
									getEffectiveTags={getEffectiveTags}
									alwaysVisible
						...
						<button
							type="button"
							className="icon-button delete-btn modal-delete-floating"
							aria-label="Delete clip"
							title="Delete clip"
							onClick={() => {
								onDelete(openClip.Id);
								closeModal();
							}}
						>
							<span className="icon icon-delete" aria-hidden="true" />
...
function TagAddControl({
	...
	getEffectiveTags?: (clip: ClipModel) => string[];
}) {
	...
	const commit = async () => {
		...
		if (effectiveTags.includes(name)) {
			setValue("");
			return;
		}
