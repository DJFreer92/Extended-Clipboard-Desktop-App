...
export function useTags() {
	...
	const addTagOptimistic = useCallback(async (clipId: number, tagName: string): Promise<boolean> => {
		...
		setOptimisticUpdates((prev) => [...prev, { clipId, tagName, operation: "add", timestamp }]);
	...
	const removeTagOptimistic = useCallback(async (clipId: number, tagName: string): Promise<boolean> => {
		...
		// Add optimistic update
		setOptimisticUpdates((prev) => [...prev, { clipId, tagName, operation: "remove", timestamp }]);
	...
	useEffect(() => {
		const cleanup = setInterval(() => {
			...
			setOptimisticUpdates((prev) => prev.filter((update) => now - update.timestamp < STALE_THRESHOLD));
