...
describe("useClipboardWatcher", () => {
	...
	it("polling path adds new clipboard content", async () => {
		const { result } = renderHook(() =>
			useClipboardWatcher({ enabled: true, deps: [], onNewClip: onNew, existingClips: existing })
		);
	...
	it("suppresses duplicate clipboard change within suppress window", async () => {
		renderHook(() =>
			useClipboardWatcher({ enabled: true, deps: [], onNewClip: onNew, existingClips: existing, guardDurationMs: 5000 })
		);
