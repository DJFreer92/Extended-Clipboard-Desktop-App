...
export function useClipboardWatcher(opts: ClipboardWatcherOptions) {
	...
	useEffect(() => {
		...
		const addClip = async (text: string) => {
			try {
				...
				try {
					appName = await (window as any)?.electronAPI?.frontmostApp?.getName?.();
				} catch {}
				if (!appName) {
					try {
						appName = await (window as any)?.electronAPI?.app?.getName?.();
					} catch {}
				}
	...
	const markSelfCopy = (text: string) => {
		...
		setTimeout(() => {
			selfInitiatedCopyRef.current = false;
		}, 400);
	};
