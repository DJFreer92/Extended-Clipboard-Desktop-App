...
function getMockData(path: string): any {
	...
	if (path.includes("/clipboard/get_num_filtered_clips")) {
		...
		if (selectedTags.length > 0) {
			filteredClips = filteredClips.filter((clip) => selectedTags.some((tag) => clip.tags.includes(tag)));
		}

		// Filter by selected apps
		if (selectedApps.length > 0) {
			filteredClips = filteredClips.filter((clip) => selectedApps.includes(clip.from_app_name));
		}
