...
describe("searchService – filtering and search", () => {
	...
	it("filterAllClips constructs proper query parameters", async () => {
		...
		expect(globalThis.fetch).toHaveBeenCalledWith(
			expect.stringContaining("/clipboard/filter_all_clips"),
			expect.any(Object)
		);
	...
	it("filterNClips includes n parameter when provided", async () => {
		...
		expect(globalThis.fetch).toHaveBeenCalledWith(
			expect.stringMatching(/filter_n_clips.*n=10.*selected_apps=AppX/),
			expect.any(Object)
		);
	});
	...
	it("filterAllClipsAfterId includes after_id parameter", async () => {
		...
		expect(globalThis.fetch).toHaveBeenCalledWith(
			expect.stringMatching(/filter_all_clips_after_id.*after_id=1.*selected_tags=tag1.*selected_apps=App1/),
			expect.any(Object)
		);
	});
	...
	it("filterNClipsBeforeId includes before_id and favorites_only parameters", async () => {
		...
		expect(globalThis.fetch).toHaveBeenCalledWith(
			expect.stringContaining("/clipboard/filter_n_clips_before_id"),
			expect.any(Object)
		);
		const url = (globalThis.fetch as any).mock.calls[0][0];
		expect(url).toContain("search=a+b");
