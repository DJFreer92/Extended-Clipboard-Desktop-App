...
export const tagsService = {
	async addClipTag(clipId: number, tagName: string): Promise<void> {
		await http<void>(
			`/clipboard/add_clip_tag?clip_id=${encodeURIComponent(clipId)}&tag_name=${encodeURIComponent(tagName)}`,
			{ method: "POST" }
		);
	},
	...
	async removeClipTag(clipId: number, tagId: number): Promise<void> {
		await http<void>(
			`/clipboard/remove_clip_tag?clip_id=${encodeURIComponent(clipId)}&tag_id=${encodeURIComponent(tagId)}`,
			{ method: "POST" }
		);
	},
	...
	async getNumClipsPerTag(tagId: number): Promise<number> {
		const res = await http<TagCountResponse | number>(
			`/clipboard/get_num_clips_per_tag?tag_id=${encodeURIComponent(tagId)}`
		);
